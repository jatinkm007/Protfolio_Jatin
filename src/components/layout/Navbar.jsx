...
const Navbar = () => {
  ...
  return (
    <nav 
    className={`fixed top-0 left-0 right-0 w-full z-1000 py-4 transition-all duration-300 ${isScrolled ? 
    ...
    style= {{transform: 'translate3d(0, 0, 0)'}}>
      ...
      <div className="max-w-[1320px] mx-auto px-5">
        <div className="flex items-center justify-between">
          ...
          <div className="hidden md:flex items-center gap-2">
            ...
            className="px-7 py-3.5 cursor-pointer bg-linear-to-r from-primary/80 via-primary/40 to-primary/10 text-[#212121] font-medium text-base rounded-[17px] border border-white hover:bg-white/90 transition-all duration-300"> 
              Hire Me
            </button>
          </div>
          ...
          <button className="md:hidden p-4 text-white hover:text-white/80 transition-colors" 
      ...
      className={`md:hidden transition-all duration-300 overflow-hidden  ${isMenuOpen ? 'max-h-96 opacity-100' : 'max-h-0 opacity-0'}`}>
        ...
        <div className="bg-black/95 backdrop-blur-lg border-t border-white/10 px-5 py-6 space-y-3">
            ...
            <button
            onClick = {() => handleNavClick('contact')}
            className = 'w-full px-7 py-3.5 bg-white text-gray-600 font-medium text-base rounded-lg border border-white hover:bg-white/90 transition-all duration-300 mt-2 '
            >
              Hire me
              </button>
        </div>
